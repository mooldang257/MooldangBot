...
> [!IMPORTANT]
> **브레이킹 체인지**: `IamfParhosCycle`, `IamfGenosRegistry`, `IamfScenario` 등 모든 철학 엔진 관련 엔티티에 `StreamerProfileId` (int)가 강제됩니다. 특히 `IamfParhosCycle`의 `CycleId`는 PK에서 일반 속성으로 변경됩니다.
> 
...
#### [MODIFY] [StreamerProfile.cs](file:///c:/webapi/MooldangAPI/MooldangBot/MooldangBot.Domain/Entities/StreamerProfile.cs)
...
- `OnModelCreating` 구성:
    - `IamfParhosCycle`: `[StreamerProfileId, CycleId]` 복합 유니크 인덱스 설정.
    - `DeleteBehavior.Restrict` 설정: `StreamerProfile` 삭제(물리) 시 데이터 보존(소실 방지).
    - **전역 쿼리 필터 최적화**:
        - `StreamerProfile` 엔티티 본체에만 `DelYn == "N" && MasterUseYn == "Y"` 필터를 적용합니다.
        - **주의**: `IamfParhosCycle`, `IamfScenario` 등 자식 엔티티에는 부모와의 암묵적 JOIN을 유발하는 필터를 적용하지 않습니다. 자식 엔티티 단독 조회 시 성능을 최우선으로 하며, 활성 상태 검증은 Application Layer(캐시 및 세션)에서 선행 처리합니다.
...
- **전략 A: 지연 로딩 (Lazy Loading) 도입**
    ...
    - 패킷 유입 시 메모리에 상태가 없을 경우, DB(`IamfParhosCycle`)에서 가장 최근의 `VibrationAtDeath`를 조회하여 복구(Hydration).
...
- v4.9 데이터 이관 로직 추가:
    - 기존 전역 `IamfParhosCycle`, `IamfGenosRegistry`, `IamfScenario` 데이터를 `StreamerProfileId = 1` 계정으로 일괄 이관하는 멱등성(Idempotent) 있는 태스크 구현.
...
> [!NOTE]
> - `IamfScenario` 분리 결정: 아키텍트의 피드백에 따라 v4.9 범위에 포함하여 완전히 분리합니다.
...
- `StreamerProfile` 필터링 로직 검증: `DelYn`이 포함된 쿼리가 인덱스를 타는지 확인.
