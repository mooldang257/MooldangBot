        ...
        ILogger<AuthController> _logger) : ControllerBase
    {
        ...
        private string BaseDomain 
        {
            get {
                ...
                throw new Exception("[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à]: ?òÍ≤Ω ?§Ï†ï ?åÏùº(.env ?êÎäî appsettings)?êÏÑú 'BASE_DOMAIN'???§Ï†ï?òÏñ¥ ?àÏ? ?äÏäµ?àÎã§.");
            }
        }
        ...
        public async Task<IActionResult> ChzzkLogin([FromQuery] string? type)
        {
            ...
            {
                _logger.LogError(ex, "[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à] Î°úÍ∑∏??URL ?ùÏÑ± ?§Ìå®");
                return Ok(Result<object>.Failure($"Î°úÍ∑∏??URL ?ùÏÑ± ?§Ìå®: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> GetMyProfile([FromQuery] string? uid)
        {
            ...
            {
                return Ok(Result<object>.Failure("?∏Ï¶ù?òÏ? ?äÏ? ?¨Ïö©?êÏûÖ?àÎã§."));
            }
            ...
            {
                return Ok(Result<object>.Failure("ÏπòÏ?Ïß?Í≥ÑÏ†ï ?∞Îèô ?ïÎ≥¥Í∞Ä ?ÜÏäµ?àÎã§."));
            }
            ...
            try 
            {
                ...
                if (profile != null)
                {
                    ...
                    return Ok(Result<object>.Success(new {
                        ...
                        channelName = profile.ChannelName ?? "?§Ìä∏Î¶¨Î®∏",
            ...
            catch (Exception ex)
            {
                ...
                return Ok(Result<object>.Failure($"?ïÎ≥¥ Í∞±Ïã† ?§Ìå®: {ex.Message}"));
            }

            return Ok(Result<object>.Failure("?ÑÎ°ú???ïÎ≥¥Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§."));
        }

        [HttpGet("auth/resolve-slug/{slug}")]
        public async Task<IActionResult> ResolveStreamerSlug(string slug)
        {
            if (string.IsNullOrEmpty(slug)) return BadRequest("[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à] ?†Ìö®?òÏ? ?äÏ? Ï£ºÏÜå?ÖÎãà??");

            var chzzkUid = await _identityCache.GetChzzkUidBySlugAsync(slug);

            if (string.IsNullOrEmpty(chzzkUid))
            {
                return Ok(Result<object>.Failure("[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à] Ï°¥Ïû¨?òÏ? ?äÎäî Ï£ºÏÜå?ÖÎãà??"));
            }
        ...
        {
            if (string.IsNullOrEmpty(slug)) return Ok(Result<object>.Failure("[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à] ?†Ìö®?òÏ? ?äÏ? Ï£ºÏÜå?ÖÎãà??"));
            ...
            return Ok(Result<object>.Failure("[?§ÏãúÎ¶¨Ïä§??Í±∞Ï†à] ?¥Îãπ Ï±ÑÎÑê??Í¥ÄÎ¶?Í∂åÌïú???ÜÏäµ?àÎã§."));
        }
        ...
        public async Task<IActionResult> BotLogin([FromQuery] string? uid)
        {
            ...
            {
                return Content($"[Î¥??∏Ï¶ù ?§Î•ò] {ex.Message}");
            }
        }

        [HttpGet("/api/auth/callback")]
        [HttpGet("/api/v1/auth/callback")]
        [HttpGet("/Auth/callback")] // ?åü [Aegis Bridge]: Nginx ?∞Ìöå Í≤ΩÎ°ú ÏßÄ??(404 Î∞©Ï?)
        [AllowAnonymous]
        public async Task<IActionResult> AuthCallback([FromQuery] string? code, [FromQuery] string? state)
        {
            if (string.IsNullOrEmpty(code) || string.IsNullOrEmpty(state)) 
            {
                return Ok(Result<object>.Failure("?ÑÏàò ?∏Ï¶ù ?åÎùºÎØ∏ÌÑ∞Í∞Ä ?ÑÎùΩ?òÏóà?µÎãà??"));
            }

            var stateFromCookie = Request.Cookies[StateCookieName];
            if (string.IsNullOrEmpty(stateFromCookie) || stateFromCookie != state)
            {
                return Ok(Result<object>.Failure("?∏Ï¶ù ?∏ÏÖò???†Ìö®?òÏ? ?äÍ±∞??Î≥ÄÏ°∞Îêò?àÏäµ?àÎã§. ?§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??"));
            }

            var cachedJson = await _cache.GetStringAsync($"auth:state:{state}");
            if (string.IsNullOrEmpty(cachedJson))
            {
                return Ok(Result<object>.Failure("?∏Ï¶ù ?úÍ∞Ñ??Ï¥àÍ≥º?òÏóà?µÎãà?? ?§Ïãú Î°úÍ∑∏?∏Ìï¥ Ï£ºÏÑ∏??"));
            }

            var cachedData = JsonSerializer.Deserialize<AuthSessionData>(cachedJson);
            if (cachedData == null) return Ok(Result<object>.Failure("?úÏä§???§Î•ò: ?∏Ï¶ù ?∏ÏÖò ?∞Ïù¥?∞Í? ?êÏÉÅ?òÏóà?µÎãà??"));

            var result = await _authService.ProcessCallbackAsync(code, cachedData);

            if (!result.IsSuccess)
            {
                return Ok(Result<object>.Failure($"?∏Ï¶ù ?§Ìå®: {result.ErrorMessage}"));
            }
            ...
            if (!string.IsNullOrEmpty(result.RedirectUrl))
            {
                string htmlResponse = $@"
                    <!DOCTYPE html>
                    <html lang='ko'>
                    <head><meta charset='UTF-8'><title>Î¥??∞Îèô ?±Í≥µ</title></head>
                    <body style='background-color:#121212; color:#00e676; display:flex; justify-content:center; align-items:center; height:100vh; font-family:sans-serif; text-align:center;'>
                        <div>
                            <h1 style='color:#0093E9;'>?éâ Î¥?Í≥ÑÏ†ï ?∞Îèô ?ÑÎ£å!</h1>
                            <p style='color:#fff;'>[{result.ChannelName}] Í≥ÑÏ†ï??Î¨ºÎ©ç ?ÑÏö© Î¥áÏúºÎ°??±Î°ù?òÏóà?µÎãà??<br>?¥Ï†ú Ï∞ΩÏùÑ ?´ÏïÑÏ£ºÏÑ∏??</p>
                        </div>
                    </body>
                    </html>";
