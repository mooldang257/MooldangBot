...
* **[Rule 8] 일제 사격(Salvo) 다중 인덱스**: 동일한 키워드(`.테스트`)가 여러 부대에 배속될 수 있도록 `(streamer_profile_id, keyword, feature_type)` 복합 유니크 인덱스를 사용한다. 단, 동일한 병과 내에서의 키워드 중복은 여전히 엄격히 금지한다.

## 6. 대규모 고부하 처리 정책 (High-Volume Optimization)
