<script lang="ts">
    ...
    // [물멍]: 인라인 테마 전용 설정 참조
    let inlineSettings = $derived(settings.Inline || {
...
{#if layout?.visible !== false && settings.showQueue !== false}
    <div 
        ...
        style="
            ...
            --queue-artist-color: {inlineSettings.ArtistColor || 'rgba(255, 255, 255, 0.6)'};
            ...
            --queue-border-width: {inlineSettings.ShowBorder !== false ? (inlineSettings.BorderWidth ?? 2.5) : 0}px;
        "
    >
...
<style>
    ...
    .inline-queue {
        ...
        border-radius: 24px;
        padding: 24px;
        background: color-mix(in srgb, var(--queue-outer-bg) calc(var(--queue-outer-opacity) * 100%), transparent);
    ...
    .inline-pill.next-1 {
        ...
        box-shadow: 0 0 20px color-mix(in srgb, var(--queue-title-color) 30%, transparent);
    }
