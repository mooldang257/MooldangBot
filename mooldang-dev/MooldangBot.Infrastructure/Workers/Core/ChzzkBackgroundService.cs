    ...
    IOptionsMonitor<WorkerSettings> optionsMonitor) : BaseHybridWorker(serviceProvider, logger, optionsMonitor, nameof(ChzzkBackgroundService))
{
    ...
    private async Task CheckLiveStatusAsync(string chzzkUid, CancellationToken ct)
    {
        try
        {
            ...
            bool hasAnySession = await db.TableSysBroadcastSessions
                .AnyAsync(s => s.CoreStreamerProfiles!.ChzzkUid == chzzkUid, ct);
            bool hasRecentSession = hasAnySession && await db.TableSysBroadcastSessions
                .AnyAsync(s => s.CoreStreamerProfiles!.ChzzkUid == chzzkUid && s.StartTime > KstClock.Now.AddDays(-7), ct);
