<script lang="ts">
    ...
    const toggleExpand = (id: number) => {
        if (expandedId === id) {
            ...
            isAdding = false;
            // 편집을 위해 원본 복사본 생성
            const origin = commands.find((c: any) => c.Id === id);
            if (origin) {
    ...
    const handleSave = (id: number) => {
        ...
        const index = commands.findIndex((c: any) => c.Id === id);
        if (index !== -1) {
    ...
    const handleAdd = () => {
        ...
        commands = [...commands, { ...newCommand, Icon: iconToSave, Id: id, LastUsed: "방금 전" }];
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] font-bold text-slate-400 flex items-center gap-1.5">
                                        ...
                                        {#if cmd.Currency === 'cheese'}
                                            <span class="flex items-center gap-0.5 text-rose-500"><Ticket size={10} /> {cmd.Cost?.toLocaleString() || 0} 치즈</span>
                                        {:else}
                                            <span class="flex items-center gap-0.5 text-amber-600"><Coins size={10} /> {cmd.Cost?.toLocaleString() || 0} 포인트</span>
                                        {/if}
                                    </p>
                                </div>
                            </button>
