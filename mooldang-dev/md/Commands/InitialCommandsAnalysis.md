...
### A. 엔티티 기본값 정의 (CoreStreamerProfiles)
`CoreStreamerProfiles` 엔티티 클래스에 하드코딩된 기본값들이 가입 시 DB 필드에 저장됩니다.
...
### 현재의 한계점
1. **데이터 파편화**: `CoreStreamerProfiles` 필드에만 명령어가 존재하고 `FuncCmdUnified` 테이블에 레코드가 없으면, 새로운 관리 화면(`commands.html`)에서 해당 명령어들이 노출되지 않습니다.
2. **동작 불일치**: `UnifiedCommandHandler`는 `FuncCmdUnified` 테이블을 SSOT(Single Source of Truth)로 사용하므로, 신규 가입한 스트리머는 이 테이블에 데이터가 생성될 때까지 기본 명령어가 작동하지 않을 수 있습니다.
...
### [Phase 1] Profile 필드와 FuncCmdUnified 동시 생성
프로필 필드를 채우는 것과 동시에, `FuncCmdUnified` 테이블에 초기 레코드 5종을 삽입합니다.
...
장기적으로는 `CoreStreamerProfiles`에서 `SongCommand`, `AttendanceCommands` 등의 필드를 완전히 제거하고 `FuncCmdUnified` 테이블만 참조하도록 리팩토링해야 합니다.
