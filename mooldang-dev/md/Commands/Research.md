...
### ⚠️ 미구현 및 주의 사항
- **출석/포인트 적립**: `CoreStreamerProfiles`에는 설정 필드가 존재하고 `md/Research.md`에는 명세되어 있으나, 현재 `Application` 레이어에 이를 처리할 실질적인 **EventHandler가 누락**되어 있습니다. (향후 `ViewerPointEventHandler` 구현 필요)
