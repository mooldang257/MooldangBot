...
| 곡 신청 (FuncSongListQueues) | 채팅 명령어/후원 기반 신청곡 큐 관리 |
...
│   ├── SonglistController.cs           ← 세션(FuncSongListSessions) 관리
...
│   ├── CoreStreamerProfiles.cs             ← 스트리머 마스터 테이블 (핵심)
│   ├── ViewerProfile.cs               ← 시청자 포인트/출석 데이터
│   ├── FuncSongListQueues.cs                   ← 신청곡 큐 엔터티
│   ├── FuncSongListSessions.cs             ← 신청곡 세션 엔터티
│   ├── StreamerCommand.cs             ← 커스텀 명령어 엔터티
│   ├── FuncSongListOmakases.cs         ← 오마카세 메뉴 아이템
│   ├── FuncRouletteMain.cs                    ← 룰렛 엔터티
│   ├── FuncRouletteItems.cs                ← 룰렛 아이템 (확률 설정)
│   ├── RouletteType.cs                ← Enum (Cheese / ChatPoint)
│   ├── SysOverlayPresets.cs               ← 오버레이 프리셋 (JSON 설정 저장)
│   ├── SysSharedComponents.cs             ← 공유 컴포넌트
│   ├── SysPeriodicMessages.cs             ← 정기 메시지 설정
│   ├── SysAvatarSettings.cs               ← 아바타 설정
│   ├── SysChzzkCategories.cs               ← 치지직 카테고리 (동기화)
│   ├── SysChzzkCategoryAliases.cs          ← 카테고리 별칭 매핑
...
flowchart TD
    ...
    subgraph SecurityLayer[보안 계층 - Defense in Depth]
        POL[Policy-based Authorization\nIAuthorizationHandler]
        PIPE[MediatR Pipeline Behavior\nAuthorizationBehavior]
    end
...
modelBuilder.Entity<FuncSongListQueues>()
    .HasQueryFilter(e => !_userSession.IsAuthenticated || e.ChzzkUid == _userSession.ChzzkUid);
```

- **적용 대상:** CoreStreamerProfiles, FuncSongListQueues, StreamerCommand, FuncSongListOmakases, FuncRouletteMain, SysPeriodicMessages, FuncSongListSessions, SysOverlayPresets, SysSharedComponents, SysAvatarSettings, ViewerProfile
...
erDiagram
    ...
    CoreStreamerProfiles ||--o{ FuncSongListQueues : "has"
    CoreStreamerProfiles ||--o{ StreamerCommand : "has"
    CoreStreamerProfiles ||--o{ FuncSongListOmakases : "has"
    CoreStreamerProfiles ||--o{ FuncRouletteMain : "has"
    CoreStreamerProfiles ||--o{ ViewerProfile : "collects"
    FuncRouletteMain ||--o{ FuncRouletteItems : "contains"
```
...
- **방송 종료 감지(Live→Offline):** `Task.WhenAll()`로 병렬 라이브 상태 체크 → 오프라인 전환 시 `SysOverlayPresets` 자동 롤백 + SignalR 브로드캐스트
