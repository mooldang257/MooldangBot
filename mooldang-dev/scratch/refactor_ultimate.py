...
# Base path for the whole solution
base_path = "/home/mooldang/projects/MooldangBot/mooldang-dev"

all_cs_files = []
for root, dirs, files in os.walk(base_path):
    # Skip build artifacts and migrations
    if any(x in root for x in ["/bin/", "/obj/", "/Migrations/"]):
        continue
...
var_names = ["db", "_db", "context", "_context", "dbContext", "scopedDb", "_dbContext", "AppDbContext", "_appDb"]
...
for file_path in all_cs_files:
    # Skip core DB context definitions
    if any(x in file_path for x in ["AppDbContext.DbSets.cs", "IAppDbContext.cs", "ICommonDbContext.cs"]):
        continue
