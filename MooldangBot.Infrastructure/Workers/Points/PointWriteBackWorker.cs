    ...
    ILogger<PointWriteBackWorker> logger) : BackgroundService
{
    ...
    private readonly AsyncRetryPolicy _retryPolicy = Policy
        ...
        .WaitAndRetryAsync(5, retryAttempt => 
            ...
            (ex, timeSpan, context) => {
                logger.LogWarning(ex, "⚠️ [WriteBack] DB 업데이트 실패. {TimeSpan}초 후 재시도합니다.", timeSpan.Seconds);
            });
    ...
    {
        logger.LogInformation("🚀 [PointWriteBackWorker] 가동 시작 (설정: {Interval}s)", CurrentSettings.IntervalSeconds);
    ...
    {
        logger.LogInformation("⏳ [PointWriteBackWorker] 종료 절차 시작 - 잔여 데이터 강제 플러시(Flush)를 시도합니다.");
    ...
    private async Task ProcessSyncAsync(CancellationToken ct)
    {
        try
        {
            ...
            logger.LogInformation("📊 [WriteBack] {Count}건의 포인트 변동분을 DB에 병합합니다.", increments.Count);
            ...
            logger.LogInformation("✅ [WriteBack] {Count}건의 포인트 동기화가 성공적으로 완료되었습니다.", increments.Count);
        }
        catch (Exception ex)
        {
            logger.LogCritical(ex, "🚨 [WriteBack] 치명적 오류 발생! 처리되지 못한 데이터는 DLQ(failed_points_queue) 검토가 필요합니다.");
